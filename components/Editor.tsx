...
const Editor: React.FC<EditorProps> = ({ isGuest, resume, setResume, onBack }) => {
  ...
  const handleGenerateSummary = async () => {
    ...
    // Use the dedicated Job Title field
          ...
          <div className="border rounded-lg p-4 bg-slate-50">
            ...
            {expandedSection === 'personal' && (
              <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <input className="border p-2 rounded" placeholder="Job Title (e.g. Software Engineer)" name="jobTitle" value={resume.personalInfo.jobTitle} onChange={handlePersonalInfoChange} />
                <input className="border p-2 rounded" placeholder="Location (City, Country)" name="location" value={resume.personalInfo.location} onChange={handlePersonalInfoChange} />
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {TEMPLATES.map(t => (
                  <button
                    key={t.id}
                    onClick={() => setActiveTemplate(t.id as TemplateType)}
                    className={`flex flex-col items-center p-3 rounded-lg border-2 transition-all ${activeTemplate === t.id
